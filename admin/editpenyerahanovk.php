...
<!--/.koding edit-->
<?php 
$ubah = mysqli_query ($koneksi, "select * from tbpenyerahanovk where IDPenyerahanOVK ='$ud1'");
...
if (isset($_POST['edt'])) {
	...
	$cek = mysqli_query($koneksi, "SELECT * from `tbpenyerahanovk` where `IDPenyerahanOVK`='$data[0]'"); //cek data yg mau diedit
	
  if (mysqli_num_rows($cek) !=0) {
		$edit = mysqli_query($koneksi, "UPDATE tbpenyerahanovk set  `IDProduksi`='$idp',`TglTerimaOVK`='$tto',`SJOVK`='$sj',`Periode`='$p' where `IDPenyerahanOVK`='$ud1'") 
		or die (mysqli_error());
...
<!--/.form input-->
 <form method="post" class="popup-form col-lg-4">
						...
						<input type="date" class="form-control form-white"   name="tto" value="<?php echo $data[2];?>" >
