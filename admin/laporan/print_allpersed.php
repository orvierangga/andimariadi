...
//mengambil data dari tabel
  $sql = mysql_query( "SELECT `barang_masuk`.`kd_barang`,`data_barang`.`nama_brg`,`data_barang`.`jenis_brg`,`data_barang`.`ket` , SUM(`jumlah_msk`) AS `total_semua` FROM `barang_masuk` LEFT JOIN `data_barang` ON `barang_masuk`.`kd_barang` = `data_barang`.`kd_barang` GROUP BY `barang_masuk`.`kd_barang` order by `barang_masuk`.`kd_barang` ASC ");
...
$judul = "DATA SEMUA DAFTAR PESEDIAAN BARANG";
$header = array(

array("label"=>"Kode Barang", "length"=>20, "align"=>"JC"),
array("label"=>"Nama Barang", "length"=>30, "align"=>"C"),
array("label"=>"Jenis Barang", "length"=>60, "align"=>"C"),
