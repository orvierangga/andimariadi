...
//mengambil data dari tabel
  //$sql = mysql_query( "SELECT `barang_keluar` .`tanggal_keluar`,`barang_keluar` .`kd_barang`,`data_barang`.`nama_brg`,`data_barang`.`jenis_brg`,`data_barang`.`ket`,`barang_keluar` .`jumlah_brg`,`barang_keluar`.`keperluan`,`barang_keluar` .`id_petugas` FROM `barang_keluar` LEFT JOIN `data_barang` ON `barang_keluar`.`kd_barang` = `data_barang`.`kd_barang` order by `barang_keluar`.`tanggal_keluar` desc ");
$sql = mysql_query( "SELECT `tanggal_keluar`,`kd_barang`,`jumlah_brg`,`keperluan`,`id_petugas` FROM `barang_keluar` order by `tanggal_keluar` desc ");
...
$judul = "DATA SEMUA BARANG KELUAR";
$header = array(
array("label"=>"Tanggal Keluar", "length"=>25, "align"=>"C"),
array("label"=>"Kode Barang", "length"=>25, "align"=>"C"),
array("label"=>"Jumlah", "length"=>20, "align"=>"C"),
array("label"=>"Keperluan", "length"=>90, "align"=>"C"),
array("label"=>"Penerima", "length"=>30, "align"=>"C"),
);
