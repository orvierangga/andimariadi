                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
                            Data Permintaan Barang <small>( <?php echo IndonesiaTgl(date('Y-m-d'));?> )</small>
                        </h1>
                        <ol class="breadcrumb">
                            <li class="active">
                                <i class="fa fa-desktop"></i> Permintaan Barang
                            </li>
                        </ol>
                    </div>
                </div>
...
if (isset($_POST['terima'])) {
  ...
  if (mysqli_num_rows($cek) !=0) {
		$edit =  mysqli_query($koneksi, "UPDATE `permintaan_barang` set `status_perm`='Diterima' where `kd_perm`='$udah'") or die(mysqli_error());
		
				if($edit) {
			echo '<script type="text/javascript">alert("Data Berhasil disimpan") </script>';
			mysqli_query($koneksi, "INSERT INTO `barang_keluar`(`kd_barang`, `jumlah_brg`,`keperluan`,`tanggal_keluar`,`status`,`kd_petugas`) 
